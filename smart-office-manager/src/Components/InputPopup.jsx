...
import "../resources/styles/InputPopup.scss";
...
class InputPopup extends Component {
  ...
  createKeys() {
    ...
    keys.push(
      <div key="backspace" className="red">
        <i onClick={() => this.backSpace()} className="material-icons">
          backspace
        </i>
      </div>
    );
    keys.push(
      <div key="clear" className="red">
  ...
  createKey(digit) {
    return (
      <div key={digit}>
        {" "}
        <span onClick={() => this.onClick(digit)}>{digit}</span>{" "}
      </div>
    );
  }
          ...
          <div className="card" ref={this.setWrapperRef}>
            <div className="container">
              <div className="header">
                <h2>Insert the number of items</h2>
                <div
              ...
              <div className="keyboard">{this.createKeys()}</div>

              <div className="footer">
                <div
                  className="button submit"
                  data-active={this.state.valid}
                  onClick={() => {
                    this.props.togglePopup();
